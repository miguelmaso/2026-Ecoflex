...
function experiment_prediction(model::PhysicalModel, data::OneCycleTest)
  y_true = data.σ
  y_pred = evaluate_stress(model, data.Δt, data.λ)
...
function experiment_prediction(model::PhysicalModel, data::QuasiStaticTest)
  y_true = data.σ
  y_pred = evaluate_stress(model, data.λ)
  return y_true, y_pred
end
...
function uncertainty_models_generator(model_builder, params, data, n_samples=100)
